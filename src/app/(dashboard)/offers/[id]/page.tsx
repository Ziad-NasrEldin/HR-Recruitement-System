...
const GRAD_REQ_KEYS: Record<string, string> = {
  ANY: "any",
  GRADUATE: "graduate",
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          { icon: Globe, label: t("form.language"), value: offer.language },
          { icon: Briefcase, label: t("form.workModel"), value: t(`workModel.${offer.workModel}`) },
          { icon: MapPin, label: t("form.location"), value: offer.location },
          { icon: Users, label: t("detail.candidates"), value: String(offer._count.leads) },
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("detail.scheduleCompensation")}</CardTitle>
        </CardHeader>
        <CardContent className="grid gap-4 sm:grid-cols-2 text-sm">
          <div className="flex gap-2">
            <Clock className="h-4 w-4 shrink-0 text-muted-foreground mt-0.5" />
            <div>
              <p className="text-muted-foreground text-xs">{t("form.workingHours")}</p>
          ...
          <div className="flex gap-2">
            <Clock className="h-4 w-4 shrink-0 text-muted-foreground mt-0.5" />
            <div>
              <p className="text-muted-foreground text-xs">{t("form.shift")}</p>
              <p>{offer.shift}</p>
            </div>
          </div>
          <div className="flex gap-2">
            ...
            <div>
              <p className="text-muted-foreground text-xs">{t("form.daysOff")}</p>
              <p>{offer.daysOff}</p>
            </div>
          </div>
          {salary && (
            <div className="flex gap-2">
              ...
              <div>
                <p className="text-muted-foreground text-xs">{t("detail.salary")}</p>
                <p>
      ...
      <Card>
        ...
        <CardContent className="grid gap-4 sm:grid-cols-2 text-sm">
          <div className="flex gap-2">
            <DollarSign className="h-4 w-4 shrink-0 text-muted-foreground mt-0.5" />
            <div>
              <p className="text-muted-foreground text-xs">{t("detail.amount")}</p>
          ...
          <div className="flex gap-2">
            <Calendar className="h-4 w-4 shrink-0 text-muted-foreground mt-0.5" />
            <div>
              <p className="text-muted-foreground text-xs">{t("detail.period")}</p>
              <p>{offer.commissionPeriodDays} {t("detail.days")}</p>
            </div>
          </div>
          <div className="flex gap-2 sm:col-span-2">
            <BadgeCheck className="h-4 w-4 shrink-0 text-muted-foreground mt-0.5" />
            <div>
              <p className="text-muted-foreground text-xs">{t("detail.graduationRequirement")}</p>
              <p>{t(`form.${GRAD_REQ_KEYS[offer.graduationRequirement] ?? "any"}`)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4 text-sm">
          <div>
            <p className="text-xs text-muted-foreground mb-1">{t("form.benefits")}</p>
            <p className="whitespace-pre-wrap">{offer.benefits}</p>
          </div>
          <div>
            <p className="text-xs text-muted-foreground mb-1">{t("form.interviewProcess")}</p>
            <p className="whitespace-pre-wrap">{offer.interviewProcess}</p>
          </div>
          {offer.requirements && (
            <div>
              <p className="text-xs text-muted-foreground mb-1">{t("form.requirements")}</p>
              <p className="whitespace-pre-wrap">{offer.requirements}</p>
            </div>
          )}
          {offer.formLink && (
            <div>
              <p className="text-xs text-muted-foreground mb-1">{t("form.applicationForm")}</p>
              <a
                href={offer.formLink}
                target="_blank"
